<x-filament-widgets::widget>
    <x-filament::section>
        <style>
            ...
            .fc-cell.is-booked {
                background-color: rgba(65, 76, 232, 0.05);
            }
            .dark .fc-cell.is-booked {
                background-color: rgba(65, 76, 232, 0.15);
            }
            ...
            .fc-date-num.is-today {
                color: #414ce8;
                text-decoration: underline;
                text-underline-offset: 4px;
            }
            .dark .fc-date-num.is-today {
                color: #818cf8;
            }
                ...
                <div class="fc-days">
                    @foreach ($calendarCells as $date)
                        ...
                        @else
                            ...
                            @endphp
                            <div
                                class="fc-cell {{ $isBooked ? 'is-booked' : '' }}"
                            >
                                <span
                                    class="fc-date-num {{ $isToday ? 'is-today' : '' }}"
                                >
                                ...
                                @if ($isBooked)
                                    <div class="fc-tags-container">
                                        @foreach ($bookedDates[$date] as $reservationData)
                                            <div
                                                class="fc-tag"
                                                style="background-color: {{ $reservationData['color'] }};"
                                            >
                                                {{ $reservationData['guest_name'] }}
                                                @if ($apartmentId === 'all')
                                                    <span class="fc-tag-sub"
                                                        >({{ $reservationData['apartment_name'] }})</span
                                                    >
                                                @endif
                                                <div class="fc-tooltip">
                                                    <strong
                                                        >{{ __('Guest') }}:</strong
                                                    >
                                                    {{ $reservationData['guest_name'] }}<br /><strong
                                                        >{{ __('Apartment') }}:</strong
                                                    >
                                                    {{ $reservationData['apartment_name'] }}
                                                </div>
                                            </div>
                                        @endforeach
                                    </div>
            ...
            <div class="fc-legend">
                @if ($apartmentId === 'all')
                    @foreach ($apartments as $apt)
                        <div class="fc-legend-item">
                            <div
                                class="fc-dot"
                                style="background-color: {{ $apartmentColors[$apt->id] ?? '#414CE8' }};"
                            ></div>
                            <span>{{ $apt->name }}</span>
                        </div>
                    @endforeach
                @else
                    <div class="fc-legend-item">
                        <div
                            class="fc-dot"
                            style="background-color: {{ $apartmentColors[$apartmentId] ?? '#414CE8' }};"
                        ></div>
                        <span
                            >{{ $apartments->firstWhere('id', $apartmentId)?->name ?? __('Booked') }}</span
                        >
                    </div>
                @endif
                <div class="fc-legend-item">
