<div class="w-full min-h-[80vh] flex flex-col pt-2 sm:pt-6"
    ...
    <section class="flex flex-col px-8 md:px-14 py-10 pb-12" id="contact-info">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
            ...
            <div class="flex flex-col p-6 py-8 rounded-2xl bg-white border border-border hover:border-purple transition-all duration-200 hover:shadow-lg hover:-translate-y-1 card-shadow">
                ...
                <a href="mailto:{{ $settings->email }}" class="text-muted hover:text-purple transition mb-1 w-fit">{{ $settings->email }}</a>
                <a href="tel:{{ str_replace(' ', '', $settings->phone) }}" class="text-muted hover:text-purple transition font-medium mb-4 w-fit">{{ $settings->phone }}</a>
                <div class="mt-auto pt-4 border-t border-border w-full">
                    <span class="text-sm text-muted font-medium">{{ __('ID (IČO):') }} {{ $settings->vat }}</span>
                </div>
            </div>
            ...
            <div class="flex flex-col p-6 py-8 rounded-2xl bg-white border border-border hover:border-purple transition-all duration-200 hover:shadow-lg hover:-translate-y-1 card-shadow">
                ...
                <div class="text-muted whitespace-pre-line mt-auto pt-2">
                    {{ $settings->address }}
                </div>
            </div>
            ...
            <div class="flex flex-col p-6 py-8 rounded-2xl bg-white border border-border hover:border-purple transition-all duration-200 hover:shadow-lg hover:-translate-y-1 card-shadow">
                ...
                <div class="flex flex-col gap-3 w-full mt-2">
                    @foreach($settings->socials ?? [] as $social)
                        <a href="{{ $social['url'] }}" target="_blank" rel="noopener noreferrer" class="flex items-center gap-3 py-2 px-3 rounded-xl bg-gray border border-border hover:bg-purplePale hover:border-purplePale text-navy transition-all w-fit pr-5">
                            <span class="w-6 text-center text-purple font-bold">{{ $social['platform'] }}</span> {{ $social['name'] }}
                        </a>
                    @endforeach
                </div>
            </div>
    ...
    <section class="flex flex-col px-8 md:px-14 py-12 md:py-16 bg-gray border-t border-border" id="faq">
        ...
        <p class="text-muted mb-8 md:mb-10 max-w-2xl">
            ...
            <a href="mailto:{{ $settings->email }}" class="relative group text-purple font-bold hover:underline ml-1">
                {{ __('Write to us') }}
